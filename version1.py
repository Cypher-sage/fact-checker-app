...
def search_claim(claim_text, context, tavily):
    ...
    try:
        # Increase results and use search depth for better consistency
        response = tavily.search(
            query, 
            max_results=8,
            search_depth="advanced",  # More thorough search
            include_raw_content=False  # Faster response
        )
...
def verify_claim(claim, search_results, client):
    ...
    # Use more content from search results for better accuracy
    results_text = "\n\n".join([
